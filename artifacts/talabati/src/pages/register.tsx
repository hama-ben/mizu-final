...
function DriverDocsUpload({ session, onComplete }: { session: VerifiedSession; onComplete: () => void }) {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500" dir="rtl">
      ...
      <div className="glass-panel rounded-3xl p-4 mb-5 bg-emerald-50/50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-800">
        <p className="text-xs text-emerald-700 dark:text-emerald-300 leading-relaxed font-medium">
          سيتم تفعيل حسابك فوراً بعد رفع الوثائق.
        </p>
      </div>
      ...
      <button onClick={handleSubmit} disabled={!allUploaded || submitting}
        ...
        {submitting
          ? <><Loader2 className="w-5 h-5 animate-spin" /> جارٍ التفعيل الفوري...</>
          : allUploaded
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="bg-primary/10 text-primary text-xs font-bold px-2 py-0.5 rounded-full">500 دج / شهر</span>
              </div>
              <p className="text-sm text-slate-500 leading-relaxed">
